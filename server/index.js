...
const prueba = i18next.use(Backend).init({
    fallbackLng: 'es',
    preload: ['es', 'en']
    // backend: {
    //     loadPath: appRootDir.get() + '/locales/{{lng}}/{{ns}}.json',
    //     addPath: appRootDir.get() + '/locales/{{lng}}/{{ns}}.missing.json'
    // }
})

app.use(i18nextMiddleware.handle(i18n))
app.use('/locales', express.static(`${appRootDir.get()}/locales`))
...
const myURL = new URL(config('graphqlUri'))
const apiProxy = proxy({
    target: `${myURL.protocol}//${myURL.host}`,
    changeOrigin: true
})
app.use(myURL.pathname, apiProxy)
app.use('/ide', apiProxy)
app.use('/ide2', apiProxy)
